```
AIM : 
detect horizontal, vertical movements and blinking, gazing and perform various cursor operations

Main Idea: 
face mesh landmarks return coordinates of various parts of the face, we use them to detect if the person is doing a particular activity and use pyautogui to perform cursor operations
...
1) for horizontal and vertical movements - we calculate the offset of iris from the eye centre and compare with neutral offset and see if it is more than a threshold and trigger the cursor operation.
